...
import {
  ...
  ChevronLeft, FileSpreadsheet, Filter, Merge, MoreHorizontal, Plus,
  RefreshCw, Search, Sparkles, TableProperties, Trash2, Undo2, Upload, ZoomIn, ZoomOut, GripVertical, CheckCircle, Info, Download
} from 'lucide-react';
import { api } from '../services/api';
import {
  sourceColumns, defaultMapping, sampleRows, rowsWithIds, errorMessage, executeExport
} from '../utils/helpers';
import { DataTable } from '../components/DataTable';
import { DocumentsBtn } from '../components/UI';

export default function CleanExcel({ notify, ask, askFormat, sharedFile, setSharedFile }) {
...
export function Stepper({ current, onSelect }) {
...
export function UploadStep({ file, onUpload, onContinue }) {
...
export function CleanStep({ rows, columns, issues, serverSession, activeTool, setActiveTool, notify, ask, onDataChanged, onRefreshIssues, onBack, onContinue }) {
  const tool = activeTool;
  const duplicateRows = React.useMemo(() => rows.filter(row => row['Serial Number'] === 'DL-4X92-001'), [rows]);
...
export function ToolCard({ icon, tone, title, count, description, action, onClick }) { return <article className="tool-card"><div className={`tool-icon ${tone}`}>{icon}</div><div><h3>{title}</h3><strong>{count} <span>{description}</span></strong></div><button className="text-button" onClick={onClick}>{action} <ArrowRight size={15} /></button></article>; }

export function ToolPanel({ tool, rows, columns, issues, serverSession, back, notify, ask, onDataChanged, onRefreshIssues }) {
...
export function PanelTop({ back, icon, title, text }) { return <div className="panel-top"><button className="back-link" onClick={back}><ChevronLeft size={17} /> All cleaning tools</button><div className="panel-heading"><span>{icon}</span><div><h3>{title}</h3><p>{text}</p></div></div></div>; }

export function ArrangeStep({ mapping, setMapping, file, columns, confidence, setConfidence, serverSession, notify, onBack, onContinue }) {
...
export function ValidateStep({ rows, recordCount, serverSession, finalReady, setFinalReady, notify, onBack, onContinue }) {
...
export function ExportStep({ rows, columns, serverSession, notify, askFormat, onRestart }) {
...
export function StepFooter({ onBack, onContinue, canContinue = true }) { 
  return <div className="step-footer">{onBack ? <button className="button button-secondary" onClick={onBack}><ArrowLeft size={17} /> Back</button> : <span /> }<button className="uiverse-continue-btn" disabled={!canContinue} onClick={onContinue}><div className="btn-arrow-bg"><ArrowRight size={17} color="white" /></div><p className="btn-text-content">Continue</p></button></div>; 
}
